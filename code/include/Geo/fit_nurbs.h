...
namespace tnurbs
{
    ...
    template<typename T, int dim>
    ENUM_NURBS global_curve_interpolate_with_ends_tangent(const Eigen::Matrix<T, dim, Eigen::Dynamic> &points, Eigen::Vector<T, dim> &D0,
        Eigen::Vector<T, dim> &D1, int degree, const std::vector<T> &params, nurbs_curve<T, dim, false, -1, -1> &nurbs)
    {
        int params_size = params.size();
    ...
    template<typename T, int dim, int parameteried_type>
    ENUM_NURBS global_curve_interpolate_with_ends_tangent(const Eigen::Matrix<T, dim, Eigen::Dynamic> &points, Eigen::Vector<T, dim> &D0, 
        Eigen::Vector<T, dim> &D1, int degree,  nurbs_curve<T, dim, false, -1, -1> &nurbs)
    {
        int points_count = points.cols();
        std::vector<T> params(points_count);
        if constexpr (parameteried_type == ENPARAMETERIEDTYPE::CHORD)

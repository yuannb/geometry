...
int main(int argc, char** argv) {
    ...
    router.POST("/echo", [](const HttpContextPtr& ctx) {
        std::string temp = ctx->body();

        json graph = json::parse(temp);

        NodeGraph ng(graph["graph"]["id"]);
        CreateNodeGraphFromJson(graph, ng);
        ng.calculate();

        json result;
        for (auto it = ng.m_curves_disc.begin(); it != ng.m_curves_disc.end(); ++it)
        {
            json j;
            j["type"] = 1;
            std::vector<double> points;
            auto ps = it->second->m_ders;
            for (auto p : ps)
            {
                Eigen::Vector3d tp = p(0, 0);
                points.push_back(tp[0]);
                points.push_back(tp[1]);
                points.push_back(tp[2]);
            }
            j["points"] = json(points);
            std::vector<int> indexs;
            for (auto index : it->second->m_indexs)
            {
                indexs.insert(indexs.end(), index.begin(), index.end());
            }
            j["indexs"] = json(indexs);
            result[it->first] = j;
        }

        std::string ret_str = result.dump();
        std::cout << ret_str << std::endl;

        return ctx->send(ret_str, ctx->type());
    });
